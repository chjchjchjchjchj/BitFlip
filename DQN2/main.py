...
def main(args):
    ...
    log_dir = f"/root/BitFlip/dqn_action/{length},{args.step_is_fast}"
    writer = SummaryWriter(log_dir=log_dir, comment=f"bits={args.length},model={model_name},reward={(reward_success,reward_fail)},init_e={args.epsilon},step_is_fast={args.step_is_fast}")

    # device = torch.device('cpu')
    env_name = "BitFlip"
    env = BitFlip(length=length, reward_type=reward_type, reward_success=reward_success, reward_fail=reward_fail, step_is_fast=args.step_is_fast)
