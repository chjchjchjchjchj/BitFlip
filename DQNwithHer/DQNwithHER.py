...
class DQNwithHER:
    def __init__(self, state_dim, hidden_dim, action_dim, learning_rate, gamma, epsilon, target_update, device, length, policy_type='vanila', checkpoint_dir="save_model", name=None, HER_buffer_size=10000, minimal_epsilon=0.1, delta_epsilon=1e-5):
        """
    ...
    def store_experience(self, state, action, reward, next_state, done, goal):
        self.HER_buffer.add(state, action, reward, next_state, done, goal)
    
    def sample_experience(self, batch_size):
        return self.HER_buffer.sample(batch_size=batch_size)
